...
class Game:
    # state: intro / playing / skill_select / stage_clear / game_over
    ...
    def _burst(self, x, y, color, count=12, r=4):
    ...
    def _card_rects(self):
        ...
        total_w = n * self.CARD_W + (n - 1) * self.CARD_GAP
        start_x = SCREEN_WIDTH  // 2 - total_w // 2
        card_y  = SCREEN_HEIGHT // 2 - self.CARD_H // 2 + 30
        return [
            pygame.Rect(start_x + i * (self.CARD_W + self.CARD_GAP),
                        card_y, self.CARD_W, self.CARD_H)
            for i in range(n)
        ]
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                if self.state == "intro":
                    ...
                    idx = self._hovered_card(event.pos)
                    if idx is not None:
                        self._select_skill(self.skill_options[idx])
    ...
    def _update_playing(self):
        ...
        for enemy in self.enemies:
            ...
            self._enemy_hits_player(enemy)

        if self.boss and self.boss.alive:
            self.boss.update(self.player.x, self.player.y)
            self._enemy_hits_player(self.boss)

        self._player_bullets_vs_enemies()

        for pu in self.powerups:
            pu.update()
            if pu.alive and pu.collides_with(self.player.x, self.player.y, self.player.radius):
    ...
    def _enemy_hits_player(self, enemy):
        for bullet in enemy.bullets:
            if bullet.alive and bullet.collides_with(self.player.x, self.player.y, self.player.radius):
                bullet.alive = False
    ...
    def _player_bullets_vs_enemies(self):
        for bullet in self.player.bullets:
            if not bullet.alive:
                continue

            for enemy in self.enemies:
                if enemy.alive and bullet.collides_with(enemy.x, enemy.y, enemy.radius):
    ...
    def _draw_hud(self):
        ...
        sc = self.f_mid.render(f"SCORE {self.score:07d}", True, WHITE)
        self.screen.blit(sc, (SCREEN_WIDTH - sc.get_width() - 10, 10))

        hs = self.f_small.render(f"BEST  {self.hs.high_score:07d}", True, YELLOW)
        self.screen.blit(hs, (SCREEN_WIDTH - hs.get_width() - 10, 40))
        ...
        if self.stage.is_boss_stage:
            ...
            pw, ph   = 140, 7
            bx, by   = SCREEN_WIDTH // 2 - pw // 2, 28
            pygame.draw.rect(self.screen, GRAY, (bx, by, pw, ph), border_radius=3)
            pygame.draw.rect(self.screen, CYAN, (bx, by, int(pw * ratio), ph), border_radius=3)
            cnt = self.f_tiny.render(f"{self.stage.enemies_killed}/{self.stage.kill_quota}", True, LIGHT_GRAY)
        ...
        if self.boss and self.boss.alive:
            ...
            pygame.draw.rect(self.screen, bc, (bx, by, int(bw * self.boss.hp / self.boss.max_hp), bh))
            pygame.draw.rect(self.screen, WHITE, (bx, by, bw, bh), 1)
            lbl = self.f_small.render(f"BOSS   {self.boss.hp} / {self.boss.max_hp}", True, WHITE)
            self.screen.blit(lbl, (SCREEN_WIDTH // 2 - lbl.get_width() // 2, by - 20))
    ...
    def _draw_skill_select(self):
        ...
        self._shadow_text(self.f_large, "BOSS DEFEATED!", SCREEN_WIDTH // 2, 80, YELLOW)
        sub = self.f_mid.render("Choose an Upgrade", True, CYAN)
        self.screen.blit(sub, (SCREEN_WIDTH // 2 - sub.get_width() // 2, 128))

        for i, (sid, rect) in enumerate(zip(self.skill_options, self._card_rects())):
            ...
            if hovered:
                ...
                glow  = pygame.Surface((rect.w + g_pad * 2, rect.h + g_pad * 2), pygame.SRCALPHA)
                pygame.draw.rect(glow, (*color, int(70 * pulse)),
                                 (0, 0, rect.w + g_pad * 2, rect.h + g_pad * 2), border_radius=20)
                self.screen.blit(glow, (rect.x - g_pad, rect.y - g_pad))

            bg = (42, 42, 66) if hovered else (24, 24, 40)
            pygame.draw.rect(self.screen, bg, rect, border_radius=14)

            pygame.draw.rect(self.screen, color,
                             pygame.Rect(rect.x, rect.y, rect.w, 64), border_radius=14)
            pygame.draw.rect(self.screen, color,
                             (rect.x, rect.y + 50, rect.w, 14))

            name_s = self._fit_text(self.f_mid, self.f_small, sdef["name"], inner_w)
            self.screen.blit(name_s, (rect.x + rect.w // 2 - name_s.get_width() // 2, rect.y + 14))

            tag_s  = self.f_tiny.render(sdef["tag"], True, (20, 20, 20))
            tx, ty = rect.x + rect.w // 2 - tag_s.get_width() // 2, rect.y + 46
            badge  = pygame.Surface((tag_s.get_width() + 10, tag_s.get_height() + 3), pygame.SRCALPHA)
            badge.fill((0, 0, 0, 90))
            self.screen.blit(badge, (tx - 5, ty - 1))
            self.screen.blit(tag_s, (tx, ty))

            desc_lines = self._wrap(self.f_tiny, sdef["desc"], inner_w)
            line_h     = self.f_tiny.get_linesize()
            desc_y     = rect.y + 80
            for line in desc_lines:
                dl = self.f_tiny.render(line, True, LIGHT_GRAY)
                self.screen.blit(dl, (rect.x + rect.w // 2 - dl.get_width() // 2, desc_y))
                desc_y += line_h
            ...
            if max_stk > 1:
                ...
                for d in range(max_stk):
                    cx = dx_start + d * (dot_r * 2 + 5) + dot_r
                    if d < stack:
                        pygame.draw.circle(self.screen, color, (cx, dot_y), dot_r)
                    else:
                        pygame.draw.circle(self.screen, GRAY,      (cx, dot_y), dot_r)
                        pygame.draw.circle(self.screen, DARK_GRAY, (cx, dot_y), dot_r - 2)
                lv = self.f_tiny.render(f"Level {stack} -> {stack + 1}", True, LIGHT_GRAY)
                self.screen.blit(lv, (rect.x + rect.w // 2 - lv.get_width() // 2, dot_y + dot_r + 4))

            border_c = WHITE if hovered else tuple(max(0, c - 70) for c in color)
            pygame.draw.rect(self.screen, border_c, rect, 3 if hovered else 2, border_radius=14)
            ...
            lbl   = self.f_small.render("CHOOSE  >", True, (0, 0, 0) if hovered else GRAY)
            self.screen.blit(lbl, (btn.x + btn.w // 2 - lbl.get_width() // 2, btn.y + 8))
    ...
    def _draw_intro(self):
        self._shadow_text(self.f_huge, "ASTRO SHOOTER", SCREEN_WIDTH // 2, 120, CYAN)
        ...
        for i, (key, val) in enumerate([
            ...
            self.screen.blit(self.f_small.render(key, True, YELLOW),
                             (SCREEN_WIDTH // 2 - 180, 265 + i * 28))
            self.screen.blit(self.f_small.render(val, True, LIGHT_GRAY),
                             (SCREEN_WIDTH // 2 - 10,  265 + i * 28))

        by3 = 390
        self.screen.blit(
            self.f_small.render("ENEMY TYPES", True, CYAN),
            (SCREEN_WIDTH // 2 - self.f_small.size("ENEMY TYPES")[0] // 2, by3))
        for i, (name, color, desc) in enumerate([
            ...
            ("BOSS",     PURPLE,          "Every 3rd stage"),
        ]):
            pygame.draw.circle(self.screen, color,
                               (SCREEN_WIDTH // 2 - 120, by3 + 22 + i * 22), 7)
            self.screen.blit(
                self.f_tiny.render(f"{name:<12} {desc}", True, LIGHT_GRAY),
                (SCREEN_WIDTH // 2 - 105, by3 + 15 + i * 22))

        hs = self.f_small.render(f"High Score:  {self.hs.high_score:07d}", True, YELLOW)
        self.screen.blit(hs, (SCREEN_WIDTH // 2 - hs.get_width() // 2, 520))

        if (pygame.time.get_ticks() // 500) % 2 == 0:
            p = self.f_mid.render("Press any key or click to start", True, WHITE)
            self.screen.blit(p, (SCREEN_WIDTH // 2 - p.get_width() // 2, 566))
    ...
    def _draw_stage_clear(self):
        ...
        nxt = self.f_mid.render(f"Entering Stage {self.stage_num}...", True, CYAN)
        self.screen.blit(nxt, (SCREEN_WIDTH // 2 - nxt.get_width() // 2, SCREEN_HEIGHT // 2 + 18))

        if self.stage.is_boss_stage:
            w = self.f_mid.render("BOSS INCOMING!", True, RED)
            self.screen.blit(w, (SCREEN_WIDTH // 2 - w.get_width() // 2, SCREEN_HEIGHT // 2 + 55))
    ...
    def _draw_game_over(self):
        ...
        self._shadow_text(self.f_huge, "GAME OVER", SCREEN_WIDTH // 2, 140, RED)
        ...
        for i, (txt, col) in enumerate([
            ...
            self.screen.blit(s, (SCREEN_WIDTH // 2 - s.get_width() // 2, 240 + i * 42))
        ...
        opt = self.f_small.render("[R]  Restart          [Q / ESC]  Quit", True, LIGHT_GRAY)
        self.screen.blit(opt, (SCREEN_WIDTH // 2 - opt.get_width() // 2, 440))

    # --- utils ---

    def _shadow_text(self, font, text, cx, cy, color, shadow=(0, 0, 0)):
        sh = font.render(text, True, shadow)
        mn = font.render(text, True, color)
        self.screen.blit(sh, (cx - mn.get_width() // 2 + 3, cy - mn.get_height() // 2 + 3))
        self.screen.blit(mn, (cx - mn.get_width() // 2,     cy - mn.get_height() // 2))
    ...
    @staticmethod
    def _fit_text(font_big, font_small, text, max_w):
        s = font_big.render(text, True, (0, 0, 0))
        return s if s.get_width() <= max_w else font_small.render(text, True, (0, 0, 0))
